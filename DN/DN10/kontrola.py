def preberi_zemljevid(ime_dat):
    map = [row.strip() for row in open(ime_dat)]
...
def map_distance(x, y, map, prepovedani):
    map_new = {}

    for char_coordinates, char in map.items():
        if char_coordinates in prepovedani:
            continue
        char_distance = distance((x, y), char_coordinates)
        if char not in map_new or map_new[char][1] > char_distance:
            map_new[char] = [char_coordinates, char_distance]

    return map_new


def najblizji(x, y, c, zemljevid, prepovedani):
    map_dis = map_distance(x, y, zemljevid, {(x, y)})
    if c and c in map_dis:
        return map_dis[c][0]

    max_k, max_v = min(map_dis.items(), key=lambda x: x[1][1])
    return max_v[0]


def map_in_radar(x, y, d, zemljevid):
    char_radar = collections.defaultdict(int)

    for char_coordinate, char in zemljevid.items():
        if distance((x, y), char_coordinate) <= d:
            char_radar[char] += 1
...
def najpogostejsi(x, y, d, zemljevid):
    char_radar = map_in_radar(x, y, d, zemljevid)

    if char_radar:
        return max(dict(sorted(char_radar.items())), key=char_radar.get)
    return None


def vsi_najpogostejsi(x, y, d, zemljevid):
    map_dis = map_in_radar(x, y, d, zemljevid)

    if map_dis:
        closest_element = max(map_dis.values())
        return set(element for element, num in map_dis.items() if num == closest_element)

    return set()


def najblizji(x, y, c, zemljevid, prepovedani):
    prepovedani_new = prepovedani.copy()
    prepovedani_new.add((x, y))

    map_dis = map_distance(x, y, zemljevid, prepovedani_new)

    if c and c in map_dis:
        return map_dis[c][0]

    if c and c not in map_dis:
        return None
...
def angelca(x, y, znamenitosti, zemljevid):
    ...
    for point in znamenitosti:
        point_position = najblizji(x, y, point, zemljevid, prepovedana)
        if point_position is None:
            break

        path_len += distance((x, y), point_position)

        prepovedana.add(point_position)
        x, y = point_position
...
def next_step(pot):
    ...
    for i, c in enumerate(pot):
        if c in directions:
            step_direction = c
            return step_size, step_direction, pot[i + 1:]
        step_size = step_size * (10 * i) + int(c)


def move_position(x, y, step_size, step_direction):
    directions_x = ['<', '>']
    directions_y = ['^', 'v']

    if step_direction in directions_x:
        if step_direction == '<':
            x -= step_size
        else:
            x += step_size
...
def johanca(x, y, pot, zemljevid):
    ...
    while pot:
        step_size, step_direction, pot = next_step(pot)
        x, y = move_position(x, y, step_size, step_direction)
        if (x, y) in zemljevid and (x, y) not in visited:
            path += zemljevid[(x, y)]
            visited.add((x, y))
...
def num_of_chars(x, y, a, map):
    char_num = 0
    for x_move in range(a):
        for y_move in range(a):
            if (x + x_move, y + y_move) in map:
                char_num += 1
...
def najboljsa_cetrt(a, zemljevid):
    first_char_x = first_char_y = 0
    last_char_x = last_char_y = 0
    ...
    square_num_of_chars = None

    for x in range(first_char_x, last_char_x - a + 2):
        for y in range(first_char_y, last_char_y - a + 2):
            new_square_num_of_chars = num_of_chars(x, y, a, zemljevid)
            if square_coordinates is None or square_num_of_chars < new_square_num_of_chars:
                square_coordinates = (x, y)

                square_num_of_chars = new_square_num_of_chars

    return square_coordinates


def in_range(x, y, d, map):
    char_in_range = set()

    for char_coordinates, char in map.items():
        if distance((x, y), char_coordinates) <= d:
            char_in_range.add(char_coordinates)

    return char_in_range


def doseglive_rec(x, y, d, n, n_max, dodane, zemljevid):
    map_in_range = set()

    chars_in_range = in_range(x, y, d, zemljevid)
    chars_in_range = chars_in_range - dodane


    for (char_x, char_y) in chars_in_range:
        char = zemljevid[char_x, char_y]
        if n == 0 and char != '*':
            continue

        map_in_range = map_in_range | {char_rad for char_rad in chars_in_range if zemljevid[char_rad] != '*'}
        if char == '*':
            if n == n_max:
                continue
            if zemljevid[(x, y)] == '*':
                continue
            map_in_range = map_in_range | doseglive_rec(char_x, char_y, d, n_max, n_max, dodane | {(char_x, char_y)}, zemljevid)
        else:
            map_in_range = map_in_range | doseglive_rec(char_x, char_y, d, n - 1, n_max, dodane | {(char_x, char_y)}, zemljevid)

    return map_in_range


def dosegljive(x, y, d, n, zemljevid):
    return doseglive_rec(x, y, d, n, n, set(), zemljevid)
