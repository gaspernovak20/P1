...
def preberi_zemljevid(ime_dat):
    return [row.strip() for row in open(ime_dat)]


def najblizji(x, y, c, zemljevid, prepovedani):
    char_najblizji = None
    najblizji_oddaljenost= None

    if c != "":
        for ix in range(len(zemljevid[0])):
            for iy, row in enumerate(zemljevid):
                char = row[ix]
                if char == c:
                    char_oddaljenost = abs(x - ix) + abs(y - iy)
                    if char_oddaljenost == 0:
                        continue
                    if char_najblizji is None or najblizji_oddaljenost > char_oddaljenost:
                        char_najblizji = (ix, iy)
                        najblizji_oddaljenost = char_oddaljenost
    else:
        for ix in range(len(zemljevid[0])):
            for iy, row in enumerate(zemljevid):
                char = row[ix]
                if char != '.':
                    char_oddaljenost = math.sqrt((x - ix)**2) + math.sqrt((y - iy)**2)
                    if char_oddaljenost == 0:
                        continue
                    if char_najblizji is None or najblizji_oddaljenost > char_oddaljenost:
                        char_najblizji = (ix,iy)
...
def najpogostejsi(x, y, d, zemljevid):
    ...
    for ix in range(len(zemljevid[0])):
        for iy, row in enumerate(zemljevid):
            ...
            if char != '.':
                char_oddaljenost = math.sqrt((x - ix)**2) + math.sqrt((y - iy)**2)
                if char_oddaljenost <= d:
                    if char not in char_najpogostejsi:
                        char_najpogostejsi[char] = 0
                    char_najpogostejsi[char] += 1

    try:
        return max(dict(sorted(char_najpogostejsi.items())), key=char_najpogostejsi.get)
    except:
        return None
...
def vsi_najpogostejsi(x, y, d, zemljevid):
        ...
        rad = set()
        if char_najpogostejsi:
            naj = max(char_najpogostejsi, key=lambda k: char_najpogostejsi[k])
            for el, el_vr in char_najpogostejsi.items():
                if el_vr == char_najpogostejsi[naj]:
                    rad.add(el)
